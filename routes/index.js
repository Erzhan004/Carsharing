...
router.get('/news/:lang',cookieJwtAuth, async (req, res) => {
    try {
        let language = req.params.lang;
        console.log(language)
        const url = `https://newsapi.org/v2/everything?q=${language == 'rus' ? 'Автомобиль' : 'car'}&apiKey=e8574f4894984c0f9ee6e35db26600f7`;
        const response = await axios.get(url);

        const newsWithImages = response.data.articles.filter(article => article.urlToImage !== null);
        
        if (response.status === 200) {
            res.render('newsPage',{news: newsWithImages})
        } else {
...
router.post('/order/add/:id',cookieJwtAuth,isAdmin, async function(req,res){
...
router.post("/signin", async(req,res)=>{
    ...
    const check  = await collection.findOne({name: username});
     ...
     if(check){
        ...
        if(passcheck){
        const token = jwt.sign( {check} , "Erzhan Se-2201", { expiresIn: "1h" });
